...
deployment specifications are handled.
...
[Issue #97](https://github.com/margo/product_management/issues/97) describes expectations for a schema-agnostic approach for matching applications to devices. This SUP depends on an approved schema-agnostic, passthrough-by-default approach. The [schema-agnostic characteristic matching SUP](https://github.com/margo/specification-enhancements/blob/pdp/schema-agnostic-matching/proposals/schema-agnostic-matching.md) proposes one way of handling this.
...
- defining any new content for the Helm or Compose deployment specifications;
...
1. **Margo Core Specification:** This contains the common management and
   interoperability contract. The core specification will continue using the current [specification repository](https://github.com/margo/specification)
...
- guidelines for how deployment specifications should evolve;
...
The following table describes the initial repository and documentation
...
The following pages need to be reviewed to move or rewrite content as follows:
...
Pages retained in Core must start using deployment-neutral language such as “deployment
...
- It gives better control over the content because deployment specification repositories can have their own set of owners that control the permissions for the repository, so the Margo leadership doesn't have to be responsible for each
