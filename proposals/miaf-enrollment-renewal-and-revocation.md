...
- An MIS that accepts the certificate-form **MUST** accept it for repeated enrollments until the operator withdraws the acceptance or the certificate expires, subject to the certificate-form properties below. It is reusable, not single-use: a lapsed principal re-enrolls with the same credential ([§4](#4-renewal)). It is typically longer-lived than a token.
...
- The MIS **SHOULD** check the revocation status of the authorization certificate where the operator's PKI makes that status available. The MIS **MUST NOT** accept an authorization certificate it knows to be revoked. Handling unavailable or stale status is the operator PKI's concern. Removing the MIS-side acceptance remains the withdrawal mechanism, whether or not revocation status is available.
- The MIS **MUST** validate a presented authorization certificate against its configured trust and **MUST NOT** accept it after expiry. An operator that wants a shorter exposure uses the shortest-lived authorization certificate the operator PKI allows. Withdrawal ends acceptance without waiting for expiry or for revocation status to reach the MIS.
...
An MIS that serves several Trust Domains **MUST** keep enrollment separate for each, so that an enrollment endpoint serves exactly one Trust Domain. At renewal, the MIS **MUST** accept only the SVIDs of the Trust Domain that the enrollment endpoint serves. The rule holds under any binding, and [§8](#8-protocol-binding-est) shows how EST satisfies it, with one endpoint URL per Trust Domain. The MIS **MUST** ensure that the SPIFFE ID in every SVID it issues is well-formed, belongs to the Trust Domain the enrollment endpoint serves, and has the path-shape the applicable identity profile requires. The MIS can enforce these checks when it registers the enrollment authorization or when it issues the SVID, provided issuance preserves the validated identity. Where the MIS uses a policy front end, that front end can enforce the checks before requesting issuance from the CA. The principal still enforces that path-shape when it loads its SVID, and a verifier when it verifies a peer's SVID.
...
- The MIS **SHOULD** raise an alert when it issues for a key it has not issued for that SPIFFE ID before. The alert makes a key change visible to the operator, including an expected rekey. It does not by itself indicate unauthorized issuance. An operator that does not recognize the key revokes the certificate ([§5](#5-revocation)).

Two replica patterns occur in practice. In the first, the replicas share one key and one SVID from a secret store the platform provides. Software keys can be shared in this way; a non-exportable key bound to a device-local TPM or secure element cannot be copied to replicas on other devices. In the second, each replica is its own principal with its own SPIFFE ID under the same rules, and the accepted-client policy admits the set of instance IDs. Under both patterns, replicas present the same SPIFFE ID only where they share a single credential, because the SPIFFE ID names the principal, not the process. The pattern that gives each replica its own key under one shared SPIFFE ID is deferred ([§10](#10-roadmap-informative)).
...
Renewal repeats the issuance round-trip, authenticated by the current SVID, with no new operator-delivered input. The MIS renews only the SPIFFE ID that the current SVID carries, and overrides any name content in the renewal CSR. Routine renewal re-issues the certificate over the current key. A principal **MAY** generate a new key pair instead, for example under an operator-configured key-rotation policy. This is a rekey: the current SVID authenticates the request, and the new key signs the CSR. A rekey needs no fresh enrollment authorization and follows the key and issuance rules of [§3](#3-enrollment-flow-and-keys), including the new-key alert. A principal that has lost its current key cannot authenticate that round-trip and follows the recovery rules below.
...
The consuming rules in this section fail open by default. For an intermittently connected principal, an outage longer than a CRL validity period is a normal condition. Refusing connections without fresh revocation status would turn that outage into a loss of connectivity to otherwise trusted peers. A verifier may offer a stricter policy that the operator explicitly configures. The choice trades availability against accepting a peer whose revocation status is unknown. [§9](#9-security-considerations) records both risks.
...
- By default, a verifier **MUST NOT** refuse a connection because revocation status is unavailable, stale, or absent. A verifier **MAY** allow an operator to require usable revocation status for the peer. That policy **MUST** require explicit operator configuration. Without that configuration, a verifier that has not yet retrieved a listed CRL verifies without it until its first successful retrieval.
- A verifier **MUST** verify a CRL's signature before it applies that CRL. It **MUST** verify the signature with the certificate of the leaf's issuer, taken from the chain the peer presented, per [RFC 5280](https://datatracker.ietf.org/doc/html/rfc5280) processing. Where the issuer is a trust anchor the presenter omitted, the issuer's certificate comes from the Trust Bundle instead. If the issuer's certificate carries a `keyUsage` extension, the verifier **MUST** verify that `cRLSign` is set before it applies the CRL. Verification happens at the point of use, and not at retrieval, so a verifier needs no intermediates of its own to hold a CRL.
- A retrieved CRL **MUST NOT** replace a held, verified CRL until it has passed the signature and key-usage checks above. A verifier can hold a retrieved CRL pending those checks, but continues applying the verified copy in the meantime.
- A verifier **MUST NOT** apply a CRL that lacks the CRL Number (`cRLNumber`) extension. For the same issuer and CRL scope, it **MUST** reject a retrieved CRL whose `cRLNumber` is lower than that of the held, verified copy, and **MUST** log the rejection. An equal number leaves the held copy unchanged. Rejecting a retrieved CRL does not discard the held copy, even when that copy is past its `nextUpdate`. The comparison identifies the issuer by its name and verified signing key, not by the retrieval URL or issuer name alone.
...
- When a refreshed discovery document no longer carries `crlUris`, the verifier logs the change at once. It keeps applying its held CRLs until their `nextUpdate`, then discards them. After that, the default policy permits verification without revocation status. Removing `crlUris` **MUST NOT** disable an operator-configured requirement for usable revocation status; a verifier with that requirement refuses connections for which status is absent.
...
A revocation reaches a verifier after two delays: the MIS publishes the updated CRL, then the verifier retrieves it. With publication within 24 hours and polling every 24 hours, a revocation can remain unknown for close to 48 hours even while both endpoints are reachable and operating normally. This is not a guaranteed maximum: the polling interval is guidance, and failed retrieval can extend the delay. A deployment that needs faster propagation uses shorter publication and polling intervals.

A CRL's validity period sets the refresh deadline for a held copy; it does not guarantee when a revocation reaches a verifier. The validity period places no limit on the SVID lifetime. Where retrieval fails, the copy a verifier holds keeps applying and its status grows arbitrarily stale. The verifier logs that state, and [§9](#9-security-considerations) records the residual risk.

Two ways to go without revocation status exist, at different levels of scope. When a Trust Domain advertises no `crlUris`, it opts out for every verifier. When an operator provisions no discovery document URL to a verifier, that operator opts out for that verifier, because a verifier that never fetches the discovery document never holds the CRL list ([§6](#6-discovery)). An isolated, statically provisioned verifier is the typical case, and a constrained device in the installed mode is another. Under the default policy, both cases permit verification without revocation status, and the deployment accepts the residual risk. An operator-configured requirement for usable revocation status still applies in both cases; these opt-outs do not override it.
...
- **Enrollment**: `/simpleenroll` over TLS, server-authenticated, with a client certificate when the certificate-form enrollment authorization is used. The CSR carries no SPIFFE ID at first enrollment, because the principal does not know its SPIFFE ID yet ([§3](#3-enrollment-flow-and-keys)). The MIS takes the SPIFFE ID from the enrollment authorization, and puts it in the URI SAN of the certificate it issues ([§2](#2-the-enrollment-authorization), the exact-match rule).
  - **Authentication**: HTTP Basic, or a TLS client certificate.
    ...
    - The MIS **MUST NOT** record the Enrollment Token's secret (the HTTP Basic password field) in access logs, and **SHOULD NOT** place a TLS-terminating intermediary in front of the enrollment endpoint. [§9](#9-security-considerations) notes the exposure these prevent.
...
- **Renewal**: `/simplereenroll` over mTLS with the current SVID as client certificate. The operation carries both renewal with the current key and rekeying with a new key (RFC 7030 §4.2.2).
...
| **Rogue persistence** | An attacker holding an SVID and its private key can keep renewing while issuance remains permitted. Rekeying does not end that access: the attacker can authenticate a request for a new key of its own. | This profile places no time limit on undetected compromise. The issuance record and new-key alert ([§3](#3-enrollment-flow-and-keys)) make issuance visible, but renewal with a stolen key does not trigger the new-key alert. On detection, the operator follows the compromise-response guidance below. Once further issuance to the attacker has stopped, revocation and expiry limit the use of certificates already issued. Revocation reaches peers under the consuming rules of §5, which fail open by default unless the operator configures a stricter policy. For a rogue WFM Client, removal from the accepted-client policy remains the faster exclusion path where that policy permits it. A rogue WFM has no such path, because the WFM Identity Profile defines no accepted-server policy at the client. |
...
| **CRL rollback** | An attacker serves an older, correctly signed CRL to replace a held copy and reverse a revocation the verifier already enforces. | The [§5](#5-revocation) replacement rules require verification before replacement and reject a lower CRL Number for the same issuer and scope. A rejected retrieval leaves the held copy in force, whatever its age. This protection depends on retaining the verified copy; it cannot establish that a first retrieved CRL is the newest one, or recover knowledge lost with the cache. |
| **CRL staleness window** | A revocation is not yet visible to a verifier. Over plain HTTP, an on-path attacker can extend the window by serving the oldest still-valid CRL. Under the default fail-open policy of [§5](#5-revocation), an attacker who blocks retrieval outright holds the window open for as long as the block lasts. An attacker who controls the discovery endpoint can strip `crlUris` instead. | The [§5](#5-revocation) issuer limits: publication within 24 hours, and a validity period of 7 days or less. Once further issuance to the attacker has stopped, the SVID's own lifetime bounds the remaining certificate validity ([§4](#4-renewal)). The [§5](#5-revocation) logging rule surfaces the condition to the operator. A stripped `crlUris` field changes nothing until the held CRLs reach their `nextUpdate`, per [§5](#5-revocation). A deployment closes the replay by serving the listed CRLs over HTTPS. Removal from the accepted-client policy is unaffected. |
| **Revocation status as a denial of service** | An attacker who blocks CRL retrieval, or a CRL endpoint that fails, can stop every verifier that depends on it. Every principal class the framework adds later widens the blast radius. | [§5](#5-revocation) keeps CRL retrieval off the connection path. Under the default policy, a verifier does not refuse a connection solely because revocation status is unavailable, stale, or absent. The accepted cost is the reverse risk, recorded in the CRL staleness row above. An operator who configures a requirement for usable revocation status accepts that unavailable or stale status can prevent connections. |
| **Token exposure at the server (EST)** | The token travels as the HTTP Basic password. Access logs and TLS-terminating proxies capture such headers. | The [§8](#8-protocol-binding-est) rule prohibits recording the secret in the MIS's access logs and advises against a TLS-terminating intermediary in front of the enrollment endpoint. Expiry and single use limit what a capture yields. |
...
**Responding to a compromised principal.** The operator uses the issuance record ([§3](#3-enrollment-flow-and-keys)) to identify and revoke affected certificates, and withdraws compromised enrollment authorizations ([§2](#2-the-enrollment-authorization)). Revoking one certificate does not revoke other certificates for the same SPIFFE ID. Withdrawing an Enrollment Token does not stop renewal by an already-issued SVID.

Where the MIS supports suspension of issuance, the operator can use it during the response. This profile does not require that capability. If issuance continues, the attacker may obtain another certificate while the operator enumerates and revokes the certificates already recorded. The operator uses the CA's available controls to account for issuance during the response; one enumeration is not sufficient to establish that every affected certificate has been revoked.
