...
Update the Margo specification to state that Helm v4 charts are supported for application deployment, in addition to Helm v3. Since Helm chart format compatibility is maintained between v3 and v4; breaking changes between these versions are limited to the Helm CLI and library APIs, not the chart structure or rendering.
