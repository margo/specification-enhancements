...
* When the error is not generated by the gateway, 
    * the source for the `status.error` attribute is set to the name of the deployment as set in the `metadata.name` attribute of the ApplicationDeployment.
...
* a Workload Fleet Manager
* a "see-through" Gateway device with additional hosting capabilities (compose with 2 CPU cores and 4 Gi of memory)
* 2 devices managed by the gateway
   * device01: An IPC hosting a single node cluster (4 CPU cores and 12 Gi of memory)
   * device02: An embedded device capable of running compose application (2 CPU cores and 6 Gi of memory)

```mermaid
graph TD
    WFM[Workload Fleet Manager]
    GW[Gateway Device]
    D1[Device 01<br>Standalone Cluster]
    D2[Device 02<br>Standalone Device] 

    WFM -->|Margo API| GW
    GW -->|Proprietary API| D1
    GW -->|Proprietary API| D2
```

In this example we will look at two alternative ways to deploy applications using a see-thru gateway:

* directed deployments - The WFM will explicitly request the gateway to deploy applications on itself or on specific devices.
* autonomous deployments - The WFM will request the gateway to autonomously decide where to place deployments on itself or on its managed devices.
...
sequenceDiagram
    ...
    participant D1 as Device 01
    participant D2 as Device 02

    GW->>+WFM: POST /api/v1/onboarding
    WFM-->>-GW: Client UUID : aabbccdd-eeff-0011-2233-445566778899
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/capabilities/gateway01 <1>
    D1->>GW: Onboarding
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/capabilities/gateway01/device01 <2>
    D2->>GW: Onboarding
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/capabilities/gateway01/device02 <3>
    GW->>+WFM: GET /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments
    WFM-->>-GW: Desired State Manifest <4>
    GW->>+WFM: GET /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/bundles/sha256:b5c6d7e8f9...
    alt Directed deployments
        WFM-->>GW: Bundle for directed deployments <5>
    else Autonomous deployments
        WFM-->>-GW: Bundle for autonomous deployments <6>
        activate GW
        GW->>GW: Gateway decides where to deploy each application
        deactivate GW
    end
    GW->>+GW: Deploy applications
    GW->>-WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/a4e01b2c3d-5678-90ab-cdef-1234567890ab/status <7>
    
    GW->>+D1: Deploy applications
    D1-->>-GW: Deployment status
    activate GW
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/a3e2f5dc-912e-494f-8395-52cf3769bc06/status <8>
    deactivate GW

    GW-XD2: Deploy applications
    GW->>GW: Timeout waiting for device 02 response
    activate GW
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/b4f3g6hd-123a-456b-789c-90de1234fg56/status <9>
    deactivate GW
```

1. Gateway capabilities reporting
    ...
    ```json
    {
        ...
        "properties": {
            ...
            "modelNumber": "332ANZE1-N1",
            "serialNumber": "PF45343-AA",
            "roles": ["Gateway", "Standalone Device"],
            "resources": {
                "cpu": [
                    {
                        "cores": 2,
...
1. Device 01 capabilities reporting
...
1. Device 02 capabilities reporting
...
1. Desired state manifest
    ...
    ```json
    {
      ...
      "bundle": {
        ...
        "url": "/api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/bundles/sha256:b5c6d7e8f9..."
      },
      "deployments": [
        {
          "deploymentId": "a3e2f5dc-912e-494f-8395-52cf3769bc06",
          "digest": "sha256:a4e01b2c3d...",
          "url": "/api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/a3e2f5dc-912e-494f-8395-52cf3769bc06.sha256:a4e01b2c3d..."
        },
        {
          "deploymentId": "b4f3g6hd-123a-456b-789c-90de1234fg56",
          "digest": "sha256:c7d8e9f0a1...",
          "url": "/api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/b4f3g6hd-123a-456b-789c-90de1234fg56.sha256:c7d8e9f0a1..."
        },
        {
            "deploymentId": "c5g4h7ie-234b-567c-890d-01ef2345gh67",
            "digest": "sha256:d8e9f0a1b2...",
            "url": "/api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/c5g4h7ie-234b-567c-890d-01ef2345gh67.sha256:d8e9f0a1b2..."
        }
      ]
    }
    ```

1. Directed deployments bundle
    ...
    metadata:
      annotations:
        id: a4e01b2c3d-5678-90ab-cdef-1234567890ab
        applicationId: com-northstartida-data-aggregator
      name: data-aggregator-deployment
      namespace: default
      deviceId: gateway01
    spec:
        deploymentProfile:
            ...
            components:
                - name: data-aggregator
                  properties:
                    keyLocation: https://myhost.com/some/path/to/data-aggregator-key.asc
                    packageLocation: https://myhost.com/some/path/to/data-aggregator-package.tar.gz
        parameters:
            param:
                value: some-value
                targets:
                    - pointer: ENV.SOME_ENV_VAR
                      components:
                        - data-aggregator
    ```
    ...
    metadata:
      annotations:
        id: b4f3g6hd-123a-456b-789c-90de1234fg56
        applicationId: com-northstartida-sensor-agent
      name: sensor-agent-deployment
      namespace: default
      deviceId: gateway01/device02
    spec:
        deploymentProfile:
            type: compose
            components:
                - name: sensor-agent
                  properties:
                    keyLocation: https://myhost.com/some/path/to/sensor-agent-key.asc
                    packageLocation: https://myhost.com/some/path/to/sensor-agent-package.tar.gz
        parameters:
            param:
                value: some-value
                targets:
...
1. Autonomous deployments bundle
...
1. Deployment status reporting for deployment on gateway

    ```json
    {
        "apiVersion": "deployment.margo.org/v1alpha1",
        "kind": "DeploymentStatusManifest",
        "deploymentId": "a4e01b2c3d-5678-90ab-cdef-1234567890ab",
        "deviceId": "gateway01",
        "status": {
            "state": "deployed"
        },
...
1. Deployment status reporting for deployment on device 01
...
1. Deployment status reporting for deployment on device 02 - with error
...
### Example B - opaque gateway

For this example we will use a system composed of:

* a Workload Fleet Manager
* an opaque Gateway device with additional hosting capabilities (compose with 2 CPU cores and 4 Gi of memory)
* 2 devices managed by the gateway
   * device01: An IPC hosting a single node cluster (4 CPU cores and 12 Gi of memory)
   * device02: An embedded device capable of running compose application (2 CPU cores and 6 Gi of memory)
...
sequenceDiagram
    ...
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/capabilities/opaque-gateway01 <1>

    GW->>+WFM: GET /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments
    WFM-->>-GW: Desired State Manifest <2>
    GW->>+WFM: GET /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/bundles/sha256:b5c6d7e8f9...
    WFM-->>-GW: Bundle <3>

    GW->>+GW: Gateway decides where to deploy each application
    GW->>+GW: Deploy applications
    GW->>-WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/a4e01b2c3d-5678-90ab-cdef-1234567890ab/status <4>
    GW->>+D1: Deploy applications
    D1-->>-GW: Deployment status
    activate GW
    GW->>WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/a3e2f5dc-912e-494f-8395-52cf3769bc06/status <5>
    deactivate GW
    GW-XD2: Deploy applications
    GW->>GW: Timeout waiting for device 02 response
    GW->>-WFM: POST /api/v1/clients/aabbccdd-eeff-0011-2233-445566778899/deployments/b4f3g6hd-123a-456b-789c-90de1234fg56/status <6>
```

1. The gateway reports the combined capabilities of all its children plus its own capabilities.
    ...
    ```json
    {
        ...
        "properties": {
            ...
            "modelNumber": "332ANZE1-N1",
            "serialNumber": "PF45343-AA",
            "roles": ["Standalone Cluster", "Standalone Device"],
            "resources": {
                "cpu": [
                    {
                        "cores": 2,
...
1. Deployment status reporting for deployment on device 02 - with error
    ...
    ```json
    {
        ...
        "status": {
            ...
            "error": {
                "code": "10",
                "source": "sensor-agent-deployment",
