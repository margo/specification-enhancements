...
**The AdES signature-format family is not closed at CAdES/XAdES/PAdES/JAdES.** ETSI TS 119 152-1 V1.1.1 (2026-03) defines CB-AdES ("CBOR-AdES"), a fifth member built directly on CBOR Object Signing and Encryption (COSE, RFC 9052) — the exact base RFC this SUP's COSE_Sign1 envelope uses. CAdES (CMS/RFC 5652-based) and XAdES (XML-DSig-based) remain excludable from this SUP's envelopes by base encoding alone. JAdES and CB-AdES cannot be excluded that way: JAdES is itself built on RFC 7515 JWS, and CB-AdES is built on the same COSE this SUP profiles.

What distinguishes a JAdES or CB-AdES signature from this SUP's envelopes is not the base encoding but a defined set of mandatory qualifying header parameters neither envelope populates:

- **Claimed signing time.** CB-AdES (TS 119 152-1 §6.3 Table 14) mandates a CWT Claims header (RFC 9597) carrying `iat`, shall-be-present, cardinality exactly 1, at every baseline level (§5.1.9). JAdES (TS 119 182-1 §6.3 Table 1) mandates a "claimed signing time" service satisfied by `iat` (RFC 7519 §4.1.6, mandatory for new signatures since 2025-07-15T00:00:00Z per TS 119 182-1 §5.1.11) or, for signatures predating that cutover, `sigT` (§5.2.1). This SUP's Notary Project COSE and JWS envelopes instead carry `io.cncf.notary.signingTime` — a Notary-namespaced header, explicitly "untrusted... not used in trust decisions" under the `notary.x509` scheme, with a different wire structure. Neither envelope populates the AdES-mandated field.
- **Unsigned qualifying properties.** JAdES requires all unsigned material (validation data, archive timestamps) inside a single `etsiU` JSON array header parameter (TS 119 182-1 §5.3.1); CB-AdES uses the structurally equivalent `uHeaders` parameter. This SUP's envelopes carry no such array — timestamp and certificate-chain material live in Notary's own unprotected headers (`io.cncf.notary.timestampSignature`, `x5chain`) and OCI manifest annotations, not an ETSI-defined container.

EN 319 102-1's own scope clause (§1, V1.4.1, 2024-06) names only three formats it governs — EN 319 122-1 (CAdES), EN 319 132-1 (XAdES), EN 319 142-1 (PAdES) — and contains no reference to JAdES or CB-AdES. Both TS 119 182-1 and TS 119 152-1 state that validation procedures for their own format are outside their own document's scope and point to EN 319 102-1 for "other types" (§1, identical wording in both documents) — but EN 319 102-1's own scope clause does not, as published, incorporate either format. This SUP does not claim EN 319 102-1 governs JAdES or CB-AdES validation; neither ETSI document currently claims that either.

This SUP's envelopes are therefore not AdES signatures of any current type — not because the family's base encodings exclude COSE or JWS, but because neither envelope populates the qualifying header parameters any AdES format actually mandates. A future revision of this SUP could additionally profile CB-AdES's or JAdES's mandatory header parameters onto its existing envelopes to become a genuine, ETSI-numbered AdES format; that is a distinct, additive profiling exercise this revision does not undertake, and it would not by itself bring the signature under EN 319 102-1's validation procedures, since that document's scope does not currently cover either target format.
