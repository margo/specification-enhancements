...
**Declaring.** A principal MUST implement at least one acquisition mode and MUST declare, in its conformance documentation, which of the two modes it implements. It MAY implement both. The operator chooses for each deployment.

**Trust material (the bundle floor).** Every principal MUST accept a statically supplied Trust Bundle ([§4](#4-provisioning-input-contract) format). Every principal SHOULD also refresh it from `trustBundleUri`, over the specification's Trust Bundle API and per its retrieval rules; each accepted retrieval replaces the bundle the principal holds. A principal that implements refresh MUST accept a directly supplied `trustBundleUri`, and SHOULD also accept a discovery document URL and resolve `trustBundleUri` from it. After an offline period, a principal SHOULD refresh promptly on reconnection, so a trust-anchor rotation completed during its absence takes effect.
...
| `trustBundleUri`, or a discovery document URL resolving to it | where refresh is implemented ([§3](#3-acquisition-modes)) | the URL the principal refreshes the Trust Bundle from |
