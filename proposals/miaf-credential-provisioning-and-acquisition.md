...
flowchart BT
    ...
    subgraph L1["Consumption: acquisition modes (this SUP)"]
        ...
        c3["delivered<br/>(SPIFFE Workload API,<br/>same host)"]
    end
...
- **Delivered.**
  The principal receives its SVID, private key, and Trust Bundle over a local [SPIFFE Workload API](https://github.com/spiffe/spiffe/blob/main/standards/SPIFFE_Workload_API.md). The component serving that API is the local [SPIFFE Workload Endpoint](https://github.com/spiffe/spiffe/blob/main/standards/SPIFFE_Workload_Endpoint.md). Local here means one host: the principal and the SPIFFE Workload Endpoint run on the same host. The SPIFFE Workload Endpoint standard recommends that an endpoint instance is not exposed to more than one host, and it forbids TCP transport unless the network authenticates the workload by source address. This mode is therefore not a way to distribute credentials across a network. A principal that does not run on the same host as a SPIFFE Workload Endpoint uses the installed mode or the enrolled mode. Key generation, rotation, and the distribution of trust material happen behind that API, transparently to the principal. Because the Workload API supplies the trust material as well, the inputs for trust material in [§4](#4-provisioning-input-contract) (static bundle, `trustBundleUri`, and the initial bootstrap anchor) do not apply in this mode.
  
  The delivered mode is defined by the published SPIFFE Workload API. A proprietary local delivery mechanism does not place a principal in this mode: it would make one vendor's local agent a required component, which [§6](#6-mis-independence) forbids. A delivered principal handles no CSR, so the CSR and chain-ingestion rules below do not apply to it. The SPIFFE Workload Endpoint that holds its keys does that work instead. That component is not a Margo principal, and this SUP states no requirement for it. The deferred device identity profile covers it ([§9](#9-roadmap-and-deferred-items-informative)).
...
- **Delivered floor (delivered mode).** A principal that declares the delivered mode MUST obtain its SVID, private key, and Trust Bundle through the X.509-SVID Profile of the SPIFFE Workload API, by calling the `FetchX509SVID` method. That method returns all three in one response. A principal MAY additionally call `FetchX509Bundles`. The SPIFFE Workload Endpoint standard requires these methods to be served over gRPC, and requires a conformant client to support gRPC. This floor is therefore where the delivered mode adds a gRPC dependency. The installed and enrolled modes add none.
...
- **Device identity profile.** A WFM-independent device identity. It may be specified alongside the Device Fleet Manager work, and it touches this SUP at three points:
  - a device can fill the delivered mode for the workloads it hosts, and the profile can then state requirements for the SPIFFE Workload Endpoint that this SUP leaves unstated ([§3](#3-acquisition-modes)),
