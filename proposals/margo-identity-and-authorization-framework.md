...
| **Private Key Compromise** | An attacker exfiltrates a device's private key. | Keys **MUST** be hardware-protected (e.g., TPM, TEE, SE). MIS renewal **MUST** require fresh proof of possession (via CSR or bootstrap credential) before issuing a replacement SVID. |
